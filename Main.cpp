...
void startScreen(){
	...
	if(startG){
		//iShowBMP((startBG_x1<screenWidth? startBG_x1++ : 0), 0, "bg2.bmp");
		iShowBMP(0, 0, "bg4.1.bmp");
		iSetColor(255, 255, 255);
		iRectangle(rectangle_x, rectangle_y, rdx, rdy);
		iSetColor(start_color[0], start_color[1], start_color[2]);
		iText(start_x, start_y, "Start", GLUT_BITMAP_TIMES_ROMAN_24);
	}
	if(hScore){
		iSetColor(255, 255, 255);
		iRectangle(rectangle_x2, rectangle_y2, rdx, rdy);
		iSetColor(hScore_color[0], hScore_color[1], hScore_color[2]);
		iText(hScore_x, hScore_y, "High Score", GLUT_BITMAP_TIMES_ROMAN_24);
	}
	if(exit){
		iSetColor(255, 255, 255);
		iRectangle(rectangle_x3, rectangle_y3, rdx, rdy);
		iSetColor(xit_color[0], xit_color[1], xit_color[2]);
		iText(xit_x, xit_y, "Exit", GLUT_BITMAP_TIMES_ROMAN_24);
	}
}
...
void iDraw()
{
	...
	str2 = str3;
	iSetColor(255, 0, 0);
	iText(life_x1+40, life_y1, str2, GLUT_BITMAP_TIMES_ROMAN_24);
	if(lifeI)
